...
float quadraticWinding(float x0, float y0, float x1, float y1, float x2, float y2) {
    ...
    float w0 = saturate(y0), w2 = saturate(y2), w = 0.0, ay, by, cy, ax, bx, div2A, t, w1, r, s, x, y, tx, ty;
    ...
    if (w0 != w1) {
        y = 0.5 * (w0 + w1), cy = y0 - y, r = copysign(sqrt(max(0.0, by * by - 4.0 * ay * cy)), y1 - y0);
        t = r > 0.0 ? 2.0 * cy / (-by - r) : (-by + r) * div2A;
//        x = fma(fma(ax, t, bx), t, x0), tx = 2.0 * ax * t + bx, ty = 2.0 * ay * t + by;
//        w += winding(x - tx, y - ty, x + tx, y + ty, w0, w1);
        
        s = 1.0 - t, w += winding(s * x0 + t * x1, s * y0 + t * y1, s * x1 + t * x2, s * y1 + t * y2, w0, w1);
    }
    if (w1 != w2) {
        y = 0.5 * (w1 + w2), cy = y0 - y, r = copysign(sqrt(max(0.0, by * by - 4.0 * ay * cy)), y2 - y1);
        t = r > 0.0 ? 2.0 * cy / (-by - r) : (-by + r) * div2A;
//        x = fma(fma(ax, t, bx), t, x0), tx = 2.0 * ax * t + bx, ty = 2.0 * ay * t + by;
//        w += winding(x - tx, y - ty, x + tx, y + ty, w1, w2);
