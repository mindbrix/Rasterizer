                                ...
                                uint vid [[vertex_id]], uint iid [[instance_id]])
{
    ...
    int ic = edge.ic, ue1 = (ic & Edge::ue1) >> 22, segcount = (ue1 & 0x7) - (ue1 & 0x8 ? 1 : 0);
    ...
    p = pt + (idx < 2 && edge.prev ? edge.prev - 1: -min(2, idx)), x16 = p->x & 0x7FFF, y16 = p->y & 0x7FFF;
    ppx = x16 * ma + y16 * mc + tx, ppy = x16 * mb + y16 * md + ty;
    ...
    p = pt + (idx > segcount - 2 && edge.next ? edge.next + 1 : min(2, segcount - idx)), x16 = p->x & 0x7FFF, y16 = p->y & 0x7FFF;
...
//            alpha = ts.x;
//
//            alpha = ts.w == 1 || d0 < d1 ? ts.x : ts.y;
//            alpha = ts.w != 1 && d1 > d0 ? ts.y : 0.0;
//            alpha = 1|| ts.w == 1 ? ts.x : max(ts.x, ts.y);
//            dist = pointDistance(float2(x0, y0), float2(x1, y1), float2(x2, y2), alpha);
//            dist = tangentDistance(float2(x0, y0), float2(x1, y1), float2(x2, y2), saturate(alpha));
       } else
            dist = vert.dm;
