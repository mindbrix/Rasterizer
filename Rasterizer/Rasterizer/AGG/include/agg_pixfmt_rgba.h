...
namespace agg
{
    ...
    public:
        ...
        {
            if (!c.is_transparent()) {
                auto is_opaque = c.is_opaque();
                pixel_type* p = pix_value_ptr(x, y, len);
                if (is_opaque && cover == cover_mask) {
                    memset_pattern4(p, & c, len << 2);
                } else {
                    ...
                    if (is_opaque)
                        CGSColorMaskCopyARGB8888(col, (uint32_t *)p, 0, covers, 0, len, 1);
                    else
                        CGSColorMaskSoverARGB8888(col, (uint32_t *)p, 0, covers, 0, len, 1);
                }
            }
        }
                               ...
                               const int8u* covers)
        {
            if (!c.is_transparent()) {
                ...
                if (is_opaque)
                    CGSColorMaskCopyARGB8888(col, (uint32_t *)p, 0, (const uint8_t *)covers, 0, len, 1);
                else
                    CGSColorMaskSoverARGB8888(col, (uint32_t *)p, 0, (const uint8_t *)covers, 0, len, 1);
            }
        }

        //--------------------------------------------------------------------
        void blend_mask(int x, int y,
                               int w, int h,
                               const color_type& c,
                               const int8u* covers)
        {
            if (!c.is_transparent()) {
                auto is_opaque = c.is_opaque();
                uint32_t col = *((uint32_t *)& c);
                pixel_type* p = pix_value_ptr(x, y, w);
                
                if (is_opaque)
                    CGSColorMaskCopyARGB8888(col, (uint32_t *)p, stride(), (const uint8_t *)covers, w, w, h);
                else
                    CGSColorMaskSoverARGB8888(col, (uint32_t *)p, stride(), (const uint8_t *)covers, w, w, h);
            }
        }
