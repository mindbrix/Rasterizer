...
namespace agg
{
    ...
    void math_stroke<VC>::calc_arc(VC& vc,
                                   ...
                                   double dx2, double dy2)
    {
        float a1 = atan2f(dy1 * m_width_sign, dx1 * m_width_sign);
        float a2 = atan2f(dy2 * m_width_sign, dx2 * m_width_sign);
        float da = a1 - a2;
        int i, n;

        da = acosf(m_width_abs / (m_width_abs + 0.125 / m_approx_scale)) * 2;
        ...
        if(m_width_sign > 0)
        {
            ...
            for(i = 0; i < n; i++)
            {
                float s, c;
                __sincosf(a1, & s, & c);
                add_vertex(vc, x + c * m_width, y + s * m_width);
                a1 += da;
            }
        }
        else
                                   ...
                                   double len)
    {
        ...
        {
            float da = acosf(m_width_abs / (m_width_abs + 0.125 / m_approx_scale)) * 2;
            float a1;
            int i;
            int n = int(pi / da);
            ...
            if(m_width_sign > 0)
            {
                ...
                for(i = 0; i < n; i++)
                {
                    float s, c;
                    __sincosf(a1, & s, & c);
                    add_vertex(vc, v0.x + c * m_width, v0.y + s * m_width);
                    a1 += da;
                }
            }
            else
