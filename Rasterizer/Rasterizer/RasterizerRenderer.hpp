...
struct RasterizerRenderer {
    ...
    void renderList(Ra::SceneList& list, RasterizerState& state, Ra::Buffer *buffer) {
        ...
        buffer->prepare(list.pathsCount), buffer->useCurves = state.useCurves, buffer->fastOutlines = state.fastOutlines, buffer->p16Outlines = state.p16Outlines;
        ...
        for (size_t i = 0, iz = 0; i < list.scenes.size(); iz += list.scenes[i].count, i++)
            memcpy(ti->colors + iz, & list.scenes[i]._colors->base[0].b, list.scenes[i].count * sizeof(Ra::Colorant));
