...
struct RasterizerWinding {
    ...
    static int pointWinding(Ra::Path& path, Ra::Bounds bounds, Ra::Transform ctm, Ra::Bounds device, float dx, float dy, float w) {
        ...
        WindingInfo info;  info.dx = dx, info.dy = dy, info.dw = w * (w < 0.f ? -1.f : ws);
        ...
        if (clip.lx != clip.ux && clip.ly != clip.uy && ux >= 0.f && ux < 1.f && uy >= 0.f && uy < 1.f) {
            if (w)
                Ra::divideGeometry(path.ref, ctm, clip, false, false, false, & info, countOutline, Ra::divideQuadratic, 1.f, Ra::divideCubic, 1.f);
            else
                Ra::divideGeometry(path.ref, ctm, clip, false, true, false, & info, count, Ra::divideQuadratic, 1.f, Ra::divideCubic, 1.f);
        }
        return info.winding;
    }
};
