...
struct RasterizerWinding {
    ...
    struct WindingInfo {
        ...
        static void count(float x0, float y0, float x1, float y1, bool curve, void *info) {
            ((WindingInfo *)info)->count(x0, y0, x1, y1);
        }
        static void countOutline(float x0, float y0, float x1, float y1, bool curve, void *inf) {
                ...
                if (clip.lx != clip.ux && clip.ly != clip.uy) {
                    ...
                    if (ux >= 0.f && ux < 1.f && uy >= 0.f && uy < 1.f) {
                        if (path.ref->types.size()) {
                            if (width)
                                Ra::writePath(path.ref, ctm, clip, false, false, false, WindingInfo::countOutline, & info);
                            else
                                Ra::writePath(path.ref, ctm, clip, false, true, false, WindingInfo::count, & info);
                        } 
                    }
                }
            }
