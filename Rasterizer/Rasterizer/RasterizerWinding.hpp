...
struct RasterizerWinding {
    struct WindingInfo {
        ...
        static void countOutline(float x0, float y0, float x1, float y1, uint32_t curve, void *info) {
            if (x0 != x1 || y0 != y1) {
                ...
                if (sqrtf(cx * cx + cy * cy) < 0.5f * w->width)
                    w->winding = 1;
            }
        }
    };
    ...
    static int pointWinding(Ra::Path& path, Ra::Bounds bounds, Ra::Transform ctm, Ra::Bounds device, float dx, float dy, float w) {
        ...
        if (clip.lx != clip.ux && clip.ly != clip.uy && ux >= 0.f && ux < 1.f && uy >= 0.f && uy < 1.f) {
            if (w)
                Ra::divideGeometry(path.ref, ctm, clip, false, false, false, & info, WindingInfo::countOutline, Ra::divideQuadratic, 1.f, Ra::divideCubic, 1.f);
            else
                Ra::divideGeometry(path.ref, ctm, clip, false, true, false, & info, WindingInfo::count, Ra::divideQuadratic, 1.f, Ra::divideCubic, 1.f);
        }
        return info.winding;
    }
};
