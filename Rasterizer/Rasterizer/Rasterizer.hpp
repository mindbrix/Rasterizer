...
struct Rasterizer {
    ...
    struct Context {
        ...
        void drawPath(Path& path, AffineTransform ctm, bool even, uint8_t *src) {
            ...
            if (clip.lx != clip.ux && clip.ly != clip.uy) {
                ...
                if (stride * h < deltas.size()) {
                    ...
                    writeDeltasToBitmap(& deltas[0], stride, clip, even, src, & bitmap, nullptr);
                } else {
        ...
        void intersectClip(Path& path, AffineTransform ctm, bool even) {
            ...
            if (clip.lx != clip.ux && clip.ly != clip.uy) {
                writePath(path, ctm, clip, nullptr, 0, & segments[0]);
                writeSegments(& segments[0], clip, even, & deltas[0], clip.ux - clip.lx + 1.f, nullptr, nullptr, & clipcells[0], & clipcovers[0]);
            }
        }
    ...
    static void writeDeltasToBitmap(float *deltas, uint32_t stride, Bounds clip, bool even, uint8_t *src, Bitmap *bitmap, float *covers) {
        if (clip.lx == clip.ux)
