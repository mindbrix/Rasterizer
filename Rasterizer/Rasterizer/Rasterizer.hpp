...
struct Rasterizer {
    ...
    struct Bounds {
        static Bounds zero() { return { 0.f, 0.f, 0.f, 0.f }; }
    ...
    struct Scene {
        struct Cache {
            struct Entry {
                Entry(size_t size, bool hasMolecules, bool isFast, float *mols, uint16_t *p16s, uint8_t *p16end) : size(size), hasMolecules(hasMolecules), isFast(isFast), mols(mols), p16s(p16s), p16end(p16end) {}
                size_t size;  bool hasMolecules;  bool isFast;  float *mols;  uint16_t *p16s;  uint8_t *p16end;
            };
            size_t refCount = 0;
            ...
            if (path->types.end > 1 && (path->bounds.lx != path->bounds.ux || path->bounds.ly != path->bounds.uy)) {
                ...
                if (it != cache->map.end())
                    *(cache->ips.alloc(1)) = uint32_t(it->second);
                    ...
                    new (cache->entries.alloc(1)) Cache::Entry(path->p16s.idx, path->molecules.end > 1, path->counts[Geometry::kQuadratic] == 0 && path->counts[Geometry::kCubic] == 0, (float *)path->molecules.base, (uint16_t *)path->p16s.base, path->p16ends.base);
    ...
    struct GPU {
        ...
        struct Allocator {
            struct Pass {
                ...
                size_t cells = 0, edgeInstances = 0, fastInstances = 0, quadInstances = 0, li, ui;
            };
            void init(size_t w, size_t h) {
                full = Bounds(0.f, 0.f, w, h), sheet = strip = fast = molecules = Bounds(0.f, 0.f, 0.f, 0.f), passes.empty();
            }
            void allocAndCount(float lx, float ly, float ux, float uy, size_t idx, size_t cells, size_t edgeInstances, size_t fastInstances, size_t quadInstances, Cell *cell) {
                ...
                b->lx += w, pass->cells += cells, pass->ui++, pass->edgeInstances += edgeInstances, pass->fastInstances += fastInstances, pass->quadInstances += quadInstances;
            }
                    ...
                    for (is = clz - lz, iz = clz; iz < cuz; iz++, is++) {
                        ...
                        if (clip.lx != clip.ux && clip.ly != clip.uy) {
                            ...
                            if (width == 0.f && clip.uy - clip.ly <= kMoleculesHeight && clip.ux - clip.lx <= kMoleculesHeight) {
                                ...
                                bool fast = scene->cache->entries.base[ip].isFast || (clip.uy - clip.ly <= kFastHeight && clip.ux - clip.lx <= kFastHeight);
    ...
    static void writeCubic(float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3, Function function, void *info, float s) {
        ...
        count = a < s ? 2.f : a < 16.f ? 3.f : 2.f + floorf(sqrtf(sqrtf(a)));
    ...
    struct CurveIndexer {
        ...
        __attribute__((always_inline)) void writeCurve(float w0, float w1, float ay, float by, float y0, float ax, float bx, float x0, bool a) {
            ...
            if (fabsf(ay) < kQuadraticFlatness)
                ...
                d = by * by - 4.f * ay * (y0 - ly), t0 = ity + (d < 0.f ? 0.f : sqrtf(d) * d2a);
            tx0 = (ax * t0 + bx) * t0 + x0;
                ...
                for (cover = winding = 0.f, index = indices->base + indices->idx, lx = ux = index->x, i = begin = indices->idx; i < indices->end; i++, index++) {
                    if (index->x >= ux && fabsf(winding - roundf(winding)) < 1e-3f) {
                        if (lx != ux) {
                            ...
                            gpu.allocator.allocAndCount(lx, ly, ux, uy, gpu.blends.end - 1, 1, (i - begin + 1) / 2, 0, 0, & inst->quad.cell);
            ...
            for (gpu.ptotal = 0, j = lz = 0; j < list.scenes.size(); lz += list.scenes[j].count, j++)
                for (cache = list.scenes[j].cache.ref, puz = cache->entries.end, ip = 0; ip < puz; ip++)
                    if (gpu.fasts.base[lz + ip])
                        gpu.ptotal += cache->entries.base[ip].size;
