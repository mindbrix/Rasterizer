...
//                quadTo(x1, y1, x2, y2);
    ...
    static void splitCubic(float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3, SegmentFunction function, void *info, float precision) {
        ...
        float t = 0.f, s, xt, yt, x01, x12, x012, x23, x123, y01, y12, y012, y23, y123, ax, ay, la, cpx, cpy, mx, my;
        while (t < 1.f) {
            ...
            cpx = (3.f * (x012 + x01) - x0 - xt) * 0.25f;
            cpy = (3.f * (y012 + y01) - y0 - yt) * 0.25f;
            mx = 0.25f * (x0 + xt) + 0.5f * cpx;
            my = 0.25f * (y0 + yt) + 0.5f * cpy;
            
            (*function)(x0, y0, mx, my, 1, info), (*function)(mx, my, xt, yt, 2, info);
            
            x0 = xt, x1 = x123, x2 = x23;
            y0 = yt, y1 = y123, y2 = y23;
        }
    }
