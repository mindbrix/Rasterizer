...
struct Rasterizer {
    ...
    struct Geometry {
        ...
        static void WriteSegment16(float x0, float y0, float x1, float y1, uint32_t curve, void *info) {
            ...
            if (x0 != FLT_MAX)
                ...
                for (size_t count = 4 - (g->p16s.size() % kFastSegments); count; count--)
    ...
    static void writeContextToBuffer(SceneList& list, Context *ctx, uint32_t *idxs, size_t begin, std::vector<Buffer::Entry>& entries, Buffer& buffer) {
        ...
        if (ctx->gpu.slz != ctx->gpu.suz) {
            ...
            // ma = m.a * (path->bounds->ux - path->bounds->lx) / 32767.f;
            // mc = m.c * (path->bounds->uy - path->bounds->ly) / 32767.f;
                            for (b = path->mols, j = 0; j < scene->cache->sizes[ip]; j += kMoleculeSegments, fast++, p16 += 2 * kMoleculeSegments) {
                                if (molecules) {
                                    if (update) {
//                                      ta = ma * (b.ux - b.lx), tc = mc * (b.uy - b.ly);
            // ux = b->lx * ma + b->ly * mc + m.tx + (ta > 0.f ? ta : 0.f) + (tc > 0.f ? tc : 0.f);
