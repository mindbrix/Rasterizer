    ...
    static void writeGPUPath(Path& path, Transform ctm, bool even, uint8_t *src, size_t iz, bool unclipped, Bounds clip, bool hit, float width, Info segments, GPU& gpu) {
        if (path.ref->shapesCount) {
            ...
            if (width) {
                ...
                if (entry && !slow) {
                    size_t midx = 0, count = entry->end - entry->begin, ccount = entry->cend - entry->cbegin, molecules = path.ref->molecules.size(), instances = (count + kFastSegments - 1) / kFastSegments;
                    gpu.ctms[iz] = m;
                    if (molecules > 1) {
                        instances = 0, midx = gpu.molecules.ranges.end;
                    ...
                    writeEdges(clip.lx, clip.ly, clip.ux, clip.uy, iz, molecules, instances, true, molecules > 1 ? GPU::Instance::kMolecule : GPU::Instance::kEdge, 0.f, -int(iz + 1), entry->begin, int(midx), count, gpu);
                } else
            ...
            if ((count = segments->end - segments->idx)) {
                ...
                for (scale = 1.f / (uy - ly), cover = winding = 0.f, index = indices.base + indices.idx, lx = ux = index->x, i = begin = indices.idx; i < indices.end; i++, index++) {
                    if (index->x > ux && winding - floorf(winding) < 1e-6f) {
                        if (lx != ux)
                            writeEdges(lx, ly, ux, uy, iz, 1, (i - begin + 1) / 2, false, GPU::Instance::kEdge, cover, int(iy - ily), int(segments->idx), int(begin), i - begin, gpu);
                        begin = i;
