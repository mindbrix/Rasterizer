...
struct Rasterizer {
    struct Transform {
        ...
        inline Transform concat(Transform t, float ax, float ay) const {
            ...
            return Transform(a, b, c, d, ax, ay).concat(t).translated(-ix, -iy);
        }
        inline Transform translated(float x, float y) const { return { a, b, c, d, x * a + y * c + tx, x * b + y * d + ty }; }
                    ...
                    for (is = clz - lz, iz = clz; iz < cuz; iz++, is++) {
                        ...
                        if (clip.lx != clip.ux && clip.ly != clip.uy) {
                            ...
                            if (fast && width == 0.f) {
                                ...
                                size_t ip = scene->buffer->ips[is], i0 = scene->buffer->pi0(ip), i1 = scene->buffer->pi1(ip);
                        ...
                        *dst++ = *inst;
                        if (inst->iz & GPU::Instance::kMolecule) {
                            ...
                            for (j = i0; j < i1; j += kFastSegments, fast++) {
                                ...
                                fast->ic = uint32_t(ic), fast->i0 = j - i0, fast->i1 = ux;
                            }
