...
struct Rasterizer {
    ...
    struct Buffer {
        struct Entry {
            enum Type { kEdges, kPaints, kQuads, kOpaques };
            Entry() {}
    ...
    struct Context {
        static void writeContextsToBuffer(Context *contexts, size_t count, uint32_t *bgras, size_t pathsCount, Buffer& buffer) {
            ...
            for (i = 0; i < count; i++) {
                ...
                GPU::Index *index = context.gpu.edgeIndices.base;
        ...
        void drawPaths(Sequence *sequences, AffineTransform *ctms, bool even, uint32_t *bgras, size_t count) {
            for (size_t iz = 0; iz < count; iz++)
                if (sequences[iz].path)
                    drawPath(*sequences[iz].path, ctms[iz], even, (uint8_t *)& bgras[iz], iz);
        }
    ...
    static void writeSegments(Row<Segment> *segments, Bounds clip, bool even, uint8_t *src, size_t iz, GPU *gpu, Row<Bounds> *clipcells) {
        ...
        short counts0[256], counts1[256];
        float ly, uy, clx, cux, scale, cover, winding, lx, ux, qlx, qux, x, ox, oy;
        Segment *segment;
            ...
            if (count) {
                if (clip.ux - clip.lx < 16.f) {
                    ...
                    if (qlx != qux) {
                        ...
                        gpu->allocator.alloc(qux - qlx, ox, oy);
                        new (gpu->quads.alloc(1)) GPU::Quad(qlx, ly, qux, uy, ox, oy, iz, 0.f);
                    ...
                    for (scale = 1.f / (uy - ly), cover = winding = 0.f, index = indices.base + indices.idx, lx = ux = index->x, i = begin = indices.idx; i < indices.end; i++, index++) {
                        if (index->x > ux) {
                            ...
                            if (a == 0 || a == 255) {
                                ...
                                if (qlx != qux) {
                                    gpu->allocator.alloc(qux - qlx, ox, oy);
                                    new (gpu->quads.alloc(1)) GPU::Quad(qlx, ly, qux, uy, ox, oy, iz, cover);
