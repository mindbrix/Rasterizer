...
struct Rasterizer {
    ...
    struct GPU {
        ...
        struct Allocator {
            ...
            void allocAndCount(float lx, float ly, float ux, float uy, size_t idx, size_t cells, size_t edgeInstances, size_t fastInstances, Cell *cell) {
                        ...
                        if (clip.lx != clip.ux && clip.ly != clip.uy) {
                            ...
                            if (width == 0.f && clip.uy - clip.ly <= kMoleculesHeight && clip.ux - clip.lx <= kMoleculesHeight) {
                                ...
                                GPU::Instance *inst = new (gpu.blends.alloc(1)) GPU::Instance(iz, GPU::Instance::kMolecule | (scene->flags[is] & Scene::kFillEvenOdd ? GPU::Instance::kEvenOdd : 0) | (clip.uy - clip.ly <= kFastHeight && clip.ux - clip.lx <= kFastHeight ? GPU::Instance::kFastEdges : 0));
                                 gpu.allocator.allocAndCount(clip.lx, clip.ly, clip.ux, clip.uy, gpu.blends.end - 1, 1, 0, scene->cache->entries.base[ip].size / kFastSegments, & inst->quad.cell), inst->quad.cover = 0, inst->quad.iy = int(lz);
                            } else {
                ...
                for (cover = winding = 0.f, index = indices->base + indices->idx, lx = ux = index->x, i = begin = indices->idx; i < indices->end; i++, index++) {
                    if (index->x >= ux && fabsf(winding - roundf(winding)) < 1e-3f) {
                        if (lx != ux) {
                            ...
                            gpu.allocator.allocAndCount(lx, ly, ux, uy, gpu.blends.end - 1, 1, (i - begin + 1) / 2, 0, & inst->quad.cell);
