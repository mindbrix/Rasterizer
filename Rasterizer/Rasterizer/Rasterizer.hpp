...
struct Rasterizer {
    ...
    struct Buffer {
        struct Entry {
            enum Type { kColorants, kEdges, kQuads, kOpaques };
            Entry() {}
    ...
    struct Context {
        ...
        static void writeContextsToBuffer(Context *contexts, size_t count,
                                          ...
                                          size_t pathsCount,
                                          Buffer& buffer) {
            size_t size, i, j, begin, end, idx, qend, q, qidx, jend, iz;
            ...
            for (i = 0; i < count; i++) {
                ...
                while (ctx->gpu.quads.idx != ctx->gpu.quads.end) {
                    ...
                    for (qidx = begin, idx = j = ctx->gpu.quads.idx; j < qend; j++) {
                        if (quad[j].iz >> 24 == GPU::Quad::kRect) {
                            if (j != idx) {
                                end += (j - idx) * sizeof(GPU::Quad);
                                memcpy(buffer.data.base + qidx, ctx->gpu.quads.base + idx, end - qidx);
                                qidx = end;
                            }
                            GPU::Quad *dst = (GPU::Quad *)(buffer.data.base + qidx);
                ...
                if (ts[i] != ts[i + 1]) {
                    ...
                    if (y >= clip.ly && y < clip.uy) {
                        ...
                        if (visible) {
                            ...
                            vx = x <= clip.lx ? clip.lx : clip.ux;
                            writeLine(vx, ty0, vx, ty3, deltas, stride, segments);
                        }
                    }
                }
        }
                    ...
                    for (scale = 1.f / (uy - ly), cover = winding = 0.f, index = indices.base + indices.idx, lx = ux = index->x, i = begin = indices.idx; i < indices.end; i++, index++) {
                        if (index->x > ux && winding - floorf(winding) < 1e-6f) {
                            ...
                            if (alphaForCover(winding, even) > 0.998f) {
                                ...
                                if (lx != ux) {
                                    if (src[3] == 255 && !hit)
                                        new (gpu->opaques.alloc(1)) GPU::Quad(lx, ly, ux, uy, 0.f, 0.f, iz, 1.f, 0, 0, 0, 0);
                                    else
                                        new (gpu->quads.alloc(1)) GPU::Quad(lx, ly, ux, uy, kSolidQuad, 0.f, iz, 1.f, 0, 0, 0, 0);
                                }
                            }
                            lx = ux = index->x;
                            cover = winding;
