...
struct Rasterizer {
    ...
    struct OutlineInfo {
        ...
        static void writeInstance(float x0, float y0, float x1, float y1, void *info) {
            ...
            if (x0 == FLT_MAX) {
                if (in->dst - in->dst0 > 0) {
                    dx = in->dst0->outline.s.x0 - (in->dst - 1)->outline.s.x1;
                    dy = in->dst0->outline.s.y0 - (in->dst - 1)->outline.s.y1;
                    prev = dx * dx + dy * dy > 1e-6f ? 0 : (int)(in->dst - in->dst0 - 1);
                    in->dst0->outline.prev = prev, (in->dst - 1)->outline.next = -prev;
                }
                in->dst0 = in->dst + 1;
            }
            in->dst++;
