...
struct Rasterizer {
    ...
    struct Sequence {
        ...
        Sequence() : end(Atom::kCapacity), atomsCount(0), shapesCount(0), crc(0), px(0), py(0), bounds(FLT_MAX, FLT_MAX, -FLT_MAX, -FLT_MAX), shapes(nullptr), circles(nullptr), refCount(0), hash(0) { bzero(counts, sizeof(counts)); }
    ...
    struct GPU {
        ...
        struct Quad {
            ...
            Quad(float lx, float ly, float ux, float uy, float ox, float oy, size_t iz, int type, float cover, int iy, size_t idx, int begin, size_t count) : super(lx, ly, ux, uy, ox, oy, cover, iy, idx, begin, count), iz((uint32_t)iz | type) {}
            ...
            if (width) {
                ...
                if (e && e->begin != e->end) {
                    if (clip.uy - clip.ly > kFastHeight) {
                        ...
                        new (gpu.quads.alloc(1)) GPU::Quad(clip.lx, clip.ly, clip.ux, clip.uy, ox, oy, iz, GPU::Quad::kEdge, 0.f, im, e->begin, kNoIndices, count);
