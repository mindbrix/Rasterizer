            ...
            if (width) {
                ...
                if (fast || unclipped) {
                    ...
                    if (fast) {
                        ...
                        inst->quad.cell = gpu.allocator.allocAndCount(clip.lx, clip.ly, clip.ux, clip.uy, gpu.blends.end, path.ref->molecules.size(), 0, entry->instances), inst->quad.cover = 0, inst->quad.iy = int(entry - gpu.cache.entries.base);
                    } else {
