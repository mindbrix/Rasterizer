...
struct Rasterizer {
    ...
    struct GPU {
        ...
        */
        ...
        size_t width, height, edges, edgeInstances, outlinesCount, shapesCount;
        Allocator allocator;
    ...
    struct Context {
        ...
        static void writeContextToBuffer(Context *ctx,
            ...
            while (ctx->gpu.quads.idx != ctx->gpu.quads.end) {
                ...
                for (quad = q0; quad < qidx; quad++) {
                    qtype = quad->iz >> 24, type = qtype == GPU::Quad::kShapes || qtype == GPU::Quad::kOutlines ? Buffer::Entry::kShapes : Buffer::Entry::kQuads;
                    if (type != entry->type)
                    ...
                    if (width) {
                        ...
                        if (e && e->begin != e->end) {
                            if (clip.uy - clip.ly > kFastHeight) {
                                ...
                                gpu.edges++, gpu.edgeInstances += (count + 1) / 2;
                            }
                        } else {
                            writePath(path, ctm, clip, writeClippedSegment, sgmnts);
                            writeSegments(sgmnts.segments, clip, even, src, iz, hit, & gpu);
                    ...
                    for (scale = 1.f / (uy - ly), cover = winding = 0.f, index = indices.base + indices.idx, lx = ux = index->x, i = begin = indices.idx; i < indices.end; i++, index++) {
                        if (index->x > ux && winding - floorf(winding) < 1e-6f) {
                            if (lx != ux) {
                                ...
                                gpu->edges++, gpu->edgeInstances += (i - begin + 1) / 2;
                            }
                            begin = i;
