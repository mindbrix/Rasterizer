...
struct Rasterizer {
        ...
        void writeGPUPath(Path& path, Transform ctm, uint8_t flags, Bounds clip, float width, bool opaque, size_t iz, bool fast, bool unclipped) {
            if (width) {
                ...
                if (fast || unclipped) {
                    ...
                    if (fast) {
                        GPU::Cell cell = gpu.allocator.allocAndCount(clip.lx, clip.ly, clip.ux, clip.uy, gpu.blends.end, path.ref->molecules.size(), 0, entry->instances);
                        ...
                        inst->quad.cell = cell, inst->quad.cover = 0, inst->quad.iy = int(entry - gpu.cache.entries.base);
                    } else {
    ...
    static void writeClippedLine(float x0, float y0, float x1, float y1, Bounds clip, bool polygon, Function function, void *info) {
        float dx = x1 - x0, dy = y1 - y0, t0 = (clip.lx - x0) / dx, t1 = (clip.ux - x0) / dx, sy0, sy1, sx0, sx1, mx, vx, ts[4];
        if (dy == 0.f)
            ts[0] = 0.f, ts[3] = 1.f;
        else
