...
//                if (count == 1)
//                    Rasterizer::Context::writeContextToBuffer(& testScene.contexts[0], & testScene.scene.paths[0], & testScene.scene.ctms[0], colorants, begins[0], *buffer);
//                else {
//                    dispatch_apply(count, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^(size_t idx) {
//                        Rasterizer::Context::writeContextToBuffer(& testScene.contexts[idx], & testScene.scene.paths[0], & testScene.scene.ctms[0], colorants, b[idx], *buffer);
//                    });
//                }
                for (int i = 0; i < count; i++)
                    Rasterizer::Context::writeContextToBuffer(& testScene.contexts[i], & testScene.scene.paths[0], & testScene.scene.ctms[0], colorants, begins[i], *buffer);
            }
        }
    }
};
