...
struct RasterizerCG {
    ...
    struct ThreadInfo {
        ...
        static void drawScenes(void *info) {
            ...
            ti->context->drawList(*ti->list, ti->view, ti->paths, ti->ctms, ti->colors, ti->clips, ti->widths, ti->outlineWidth, ti->visibles, ti->slz, ti->suz, ti->bitmap, ti->tick);
        }
    ...
    static void renderScenes(Ra::SceneList& list, RasterizerState& state, size_t pathsCount, Ra::Geometry **paths, Ra::Transform *ctms, Ra::Colorant *colors, Ra::Transform *clips, float *widths, bool *visibles, float outlineWidth, Ra::Context *contexts, Ra::Bitmap *bitmap, Ra::Buffer *buffer, bool multithread, RasterizerQueue *queues) {
        ...
        if (multithread) {
            ti->context = contexts, ti->list = & list, ti->view = state.view, ti->paths = paths, ti->ctms = ctms, ti->clips = clips, ti->colors = colors, ti->widths = widths, ti->outlineWidth = outlineWidth, ti->visibles = visibles, ti->slz = 0, ti->suz = eiz, ti->bitmap = bitmap, ti->tick = buffer ? buffer->tick : 0;
            ...
            contexts[0].drawList(list, state.view, paths, ctms, colors, clips, widths, outlineWidth, visibles, 0, eiz, bitmap, buffer ? buffer->tick : 0);
        }
        if (buffer) {
    ...
    static void drawTestScene(CGTestContext& testScene, Ra::SceneList& list, RasterizerState& state, CGContextRef ctx, Ra::Bitmap *bitmap, Ra::Buffer *buffer) {
        ...
        if (testScene.rasterizerType == CGTestContext::kCoreGraphics)
            ...
            bool *_visibles = (bool *)calloc(1, pathsCount * sizeof(bool));
            ...
            renderScenes(visibles, state, pathsCount, paths, ctms, colors, clips, widths, _visibles, state.outlineWidth, & testScene.contexts[0], bitmap, buffer, testScene.rasterizerType == CGTestContext::kRasterizerMT, testScene.queues);
            free(paths), free(ctms), free(colors), free(clips), free(widths), free(_visibles);
        }
    }
};
