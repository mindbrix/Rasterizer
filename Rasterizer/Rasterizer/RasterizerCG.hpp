...
struct RasterizerCG {
    ...
    struct ThreadInfo {
        ...
        static void drawScenes(void *info) {
            ...
            ti->context->drawList(*ti->list, ti->view, ti->paths, ti->ctms, ti->colors, ti->clips, ti->widths, ti->outlineWidth, ti->slz, ti->suz, ti->bitmap, ti->tick);
        }
    ...
    static void renderScenes(Ra::SceneList& list, RasterizerState& state, size_t pathsCount, Ra::Geometry **paths, Ra::Transform *ctms, Ra::Colorant *colors, Ra::Transform *clips, float *widths, float outlineWidth, Ra::Context *contexts, Ra::Bitmap *bitmap, Ra::Buffer *buffer, bool multithread, RasterizerQueue *queues) {
        ...
        if (multithread) {
            ...
            contexts[0].drawList(list, state.view, paths, ctms, colors, clips, widths, outlineWidth, 0, eiz, bitmap, buffer ? buffer->tick : 0);
        }
        if (buffer) {
